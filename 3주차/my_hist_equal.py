...
def my_normalize_hist(hist, pixel_num):
    ...
    ########################################################
    print(hist)
    # 우리의 확률적 계산을 위해 전체 픽셀 수로 나누어 전체의 합을 1로 만들어 주기 위해서 pixel_num으로 나누어준다.
...
def my_calcHist_equalization(denormalized, hist):
    ...
    for i in range(len(denormalized)):
        changed_level = denormalized[i]
        hist_equal[changed_level] += hist[i]
...
def my_hist_equal(src, type='original'):
    ...
    ###################################################################
    plt.plot([i for i in range(256)], output_gray_level)
