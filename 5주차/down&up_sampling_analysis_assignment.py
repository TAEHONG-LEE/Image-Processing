...
def my_downsampling_pyramid(src, ratio, pyramid_lvl, filter_size, sigma, interpolation_type='nearest'):
    ...
    if filter_size is not None:
        for level in range(pyramid_lvl):
            ...
            # 2. residual 계산 및 저장
            downsampled_img = my_downsampling(filtered_img, ratio)
            if interpolation_type == 'nearest':
                upsampled_img = my_nearest_neighbor(downsampled_img, scale=(ratio, ratio))

            else:
                # bilinear interpolation
                upsampled_img = my_bilinear(downsampled_img, scale=ratio)

            laplacian_residual = gaussian_pyramid[-1] - upsampled_img
            laplacian_residuals.append(laplacian_residual)

            # 3. 2.1에서 downsampling한 이미지를 다음 피라미드의 입력으로 설정
            out_img = downsampled_img
    ...
    if filter_size is None:
        for level in range(pyramid_lvl):
            ...
            # TODO interpolation_type 변수를 활용
            ###########################################################################
            downsampled_img = my_downsampling(gaussian_pyramid[-1], ratio)
            out_img = downsampled_img
            gaussian_pyramid.append(out_img)
